...
const AddReviewCurtain = ({ toggleCurtain, submitReview}) => {
	...
	const handleSubmit = () => {
		...
		axios
			.post(`http://localhost:3002/local/create-review`, reviewForm, {
				headers: {
	...
	return (
		<div className="addReviewCurtain open">
			<div className="curtainContent">
					...
					{[...Array(5)].map((star, index) => {
						...
						return (
							<label key={index}>
								...
								<FaStar className="star2" color={ratingValue <= (hover || rating) ? "#ffc107" : "#e4e5e9"} size={30} onMouseEnter={() => setHover(ratingValue)} onMouseLeave={() => setHover(null)} style={{ cursor: "pointer" }} />
							</label>
						);
					})}
				</div>
				...
				{/* Review textarea */}
				<textarea placeholder="Write your review here..." value={reviewMessage} onChange={handleTextChange}></textarea>
...
const ReviewsBox = ({ setShowReviews, isLocalInUserLocals, reviews}) => {

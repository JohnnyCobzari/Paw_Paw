...
function LoginPage() {
	...
	useEffect(() => {
		...
		if (token) {
			...
			setAuthToken(token);
			navigate("/ServiceHome"); // Redirecționează către dashboard sau altă pagină
		}
	}, [navigate]);
	...
	const handleSubmit = async (event) => {
		...
		axios
			.post("http://localhost:3002/auth/login-local", {
				email: email,
				password: password,
			})
			.then((response) => {
	...
	return (
		<div>
			...
			<form id="login-form" onSubmit={handleSubmit}>
				...
				<div className="input_filed">
					<input type="password" className="password" placeholder="Password" value={password} onChange={handlePasswordChange} required />
				</div>
				...
				<p className="paragraf">
					...
					<a href="ServiceSignUp" className="sign-up">
						sign-up
					</a>
				</p>
			</form>

...
import MatchModal from "../Admin_PetServices/ServicesComponets/MatchModal";
		...
		<>
			...
			<div className="Profile-info">
				<div className="Profile-ImageContainer">
					<img src={petProfile.image} alt="Pet" className="Profile-ImageContainerImg"></img>
					{!isPetInUserPets && <MatchModal userPets={userPets} />}
				</div>
				<h2 className="profile-name">{petProfile.petName}</h2>
				<div className="Pet-Info">
					<p>
						<FaVenusMars /> Gender: {petProfile.gender}
					</p>
					<p>
						<FaDog /> Breed: {petProfile.breed}
					</p>
					<p>
						<FaCalendarAlt /> Age: {petProfile.age}
					</p>
					<p>
						<FaUser /> Owner: {petProfile.ownerName}
					</p>
					<p>
						<FaPhone /> Phone: {petProfile.ownerPhone}
					</p>
					<p>
						<FaSyringe /> Vaccines:
					</p>
					<ul>{petProfile.vaccines && petProfile.vaccines.map((vaccine, index) => <li key={index}>{vaccine.name}</li>)}</ul>
					<p>
						<FaAllergies /> Allergies: {petProfile.allergies}
					</p>
					<p>
						<FaClinicMedical /> Vet: {petProfile.vetInfo}
					</p>
					{petProfile.readyForBreeding && (
						<p>
							<FaDollarSign /> Breeding Price: {petProfile.breedingPrice}
						</p>
					)}
				</div>
				...
				{isPetInUserPets && (
					<div className="EditAndDeleteProfile">
						...
						<div onClick={() => setShowModal(true)}>
							<MdDelete />
							<p id="edit-infoProfile">Delete Profile</p>
						</div>
					</div>
				)}
			</div>

			{showModal && <PetProfileDeleteModal petName={petProfile.petName} onDelete={handleDelete} onCancel={handleCancel} />}
			<Footer />
		</>
	);
}
